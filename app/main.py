...
@app.post("/api/questionnaire/satisfaction")
async def recevoir_questionnaire(data: SatisfactionData):
    """Reçoit les réponses du questionnaire de satisfaction et envoie un email à MLS et Ridwan."""
    try:
        corps = f"""Questionnaire de satisfaction — Client: {data.client}

Service: {data.service}
Prestataire: {data.prestataire}
Date: {data.date if hasattr(data, 'date') else 'N/A'}

Note: {data.note}/5 — {data.noteLabel}
...
"""
        ...
        # Envoyer à MLS + Ridwan
        email_script = os.path.expanduser("~/.hermes/scripts/email/mail.py")
        for dest in ["mls@gcouca.com", "ridwan@gcouca.com"]:
            subprocess.run(
                ["python3", email_script, "send", "ridwan", dest,
                 f"📝 Questionnaire satisfaction — {data.client}",
                 tmp],
                capture_output=True, text=True, timeout=30
            )
        os.unlink(tmp)
        return {"status": "ok", "message": "Votre message a bien été envoyé"}
    except Exception as e:
        raise HTTPException(500, f"Erreur d'envoi: {str(e)}")


@app.get("/questionnaire-satisfaction", include_in_schema=False)
async def questionnaire_page():
    return FileResponse(str(STATIC_DIR / "questionnaire-satisfaction.html"))
